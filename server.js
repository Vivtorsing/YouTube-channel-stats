...
app.get('/channel-stats', async (req, res) => {
  try {
    ...
    if (input.startsWith('UC') || input.startsWith('UC_')) {
      ...
      var search = await axios.get(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&q=${input}&type=channel&key=${apiKey}`);
    ...
    //get channel data
    const response = await axios.get(`https://www.googleapis.com/youtube/v3/channels?part=statistics,snippet&id=${id}&key=${apiKey}`);
    var channelData;

    //error check again just in case we don't find anything or if the channel id is incorrect
    if (response.data.pageInfo.totalResults == 0) {
      // Channel not found
      return res.status(404).json({ error: 'Channel not found. Did you write the channel id correctly?' });
    } else {
    ...
    const channelBanner = bannerResponse.data.items[0].brandingSettings.image.bannerExternalUrl;
